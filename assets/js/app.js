...
// - Uses a SINGLE fixed redirect URI to match Cognito callback
// - Shows Google sign-in only for E‑Book; Solutions download is direct
// - Cleans up OAuth callback handling & error logging
...
class Config {
  ...
  // MUST match exactly the Cognito "Allowed callback URL" (no trailing slash)
  static REDIRECT_URI = 'https://my-ebook-assets.vercel.app';
}
...
class NavigationManager {
  ...
  showSection(sectionName) {
    ...
    if (sectionName === 'home') {
      if (this.sections.home) this.sections.home.style.display = 'flex';
    } else if (this.sections[sectionName]) {
      this.sections[sectionName].style.display = 'block';
      this.sections[sectionName].classList.add('active');
    }
...
class AuthenticationManager {
  ...
  // FIX: Use a SINGLE fixed redirect URI so it matches Cognito exactly
  redirectToGoogleSignIn() {
    const redirectUri = Config.REDIRECT_URI; // exact match with Cognito callback

    const authUrl =
      `${this.config.cognitoDomain}/oauth2/authorize?` +
      `identity_provider=Google&` +
  ...
  // Handle Cognito Hosted UI callback (code exchange)
  async handleCallback() {
    const params = new URLSearchParams(window.location.search);

    // Surface OAuth errors if any
    const err = params.get('error');
    const errDesc = params.get('error_description');
    if (err) {
      console.error('[Auth] OAuth Error:', err, errDesc);
      return false;
    }
  ...
  async exchangeCodeForTokens(code) {
    const redirectUri = Config.REDIRECT_URI; // fixed to match authorize

    try {
      const resp = await fetch(`${this.config.cognitoDomain}/oauth2/token`, {
        method: 'POST',
      ...
      if (!resp.ok) {
        console.error('[Auth] Token endpoint error:', resp.status, tokens);
        return false;
      }
      ...
      console.error('[Auth] Token exchange failed payload:', tokens);
      return false;
    } catch (e) {
      console.error('[Auth] Network/Unexpected error:', e);
      return false;
    }
  }
...
class PDFViewerManager {
  ...
  openEmbeddedViewer() {
    if (!this.ebookUrl) {
      alert('E‑Book URL not configured.');
      return;
    }
    ...
    const viewerHTML = `
      <div class="pdf-viewer-overlay" id="pdf-overlay">
        <div class="pdf-viewer-header">
          ...
          <button class="pdf-viewer-close" id="close-viewer-btn" title="Close (Esc)">×</button>
        </div>
        <div class="pdf-viewer-container" id="pdf-container-main">
          <iframe id="pdf-iframe-viewer" src="${this.ebookUrl}" frameborder="0" allowfullscreen></iframe>
        </div>
      </div>`;
...
class ErrorModal extends Modal {
  ...
  attachListeners() {
    ...
    closeBtn.onclick = () => this.remove();
    okBtn.onclick = () => this.remove();
    this.element.onclick = (e) => { if (e.target === this.element) this.remove(); };
  }
}
...
class GoogleSignInModal extends Modal {
  ...
  attachListeners() {
    ...
    const googleBtn = this.element.querySelector('#google-signin-btn');
    closeBtn.onclick = () => this.remove();
    googleBtn.onclick = () => { if (this.onSignInClick) this.onSignInClick(); };
    this.element.onclick = (e) => { if (e.target === this.element) this.remove(); };
  }
}
...
class PageComponent {
  ...
  render(content) { if (this.element) this.element.innerHTML = content; }
}

class Header extends PageComponent {
  constructor() { super('site-header'); }
  render() {
    const content = `
      <div class="header-content">
        <a href="#home" class="logo" data-section="home">
...
class Footer extends PageComponent {
  ...
  render() {
    const content = `
      ...
      <div class="footer-bottom"><p>&copy; ${new Date().getFullYear()} All rights reserved.</p></div>`;
    super.render(content);
  }
}
...
class BookCard {
  ...
  render() {
    ...
    if (this.posterElement) {
      this.posterElement.innerHTML = `
        <img src="assets/banyan_tree_cover.jpg" alt="Under the Banyan Tree - Decoding Numbers" onerror="this.src='https://via.placeholder.com/360x540/2f5d3f/ffffff?text=Decoding+Numbers'">`;
    }
    ...
    if (this.detailsElement) {
      this.detailsElement.innerHTML = `
        <div>
          ...
          <div class="meta-data">Author: Sumit Gupta | Pages: 450+ | Subject: Mathematics</div>
          <p>
            This comprehensive textbook provides an in-depth exploration of number theory concepts, designed for ISI entrance examination preparation. It covers fundamentals, advanced problem‑solving, and detailed solutions.
          </p>
        </div>
        <div class="actions">
          <button id="access-ebook-btn" class="btn btn--secondary">
            <span class="btn-icon">📖</span><span>Access E‑Book</span>
          </button>
          <button id="solutions-btn" class="btn btn--primary">
            <span class="btn-icon">📥</span><span>Solutions</span>
          </button>
        </div>`;
    }
  }
...
class AcademicPortalApp {
  ...
  async init() {
    ...
    if (authSuccess) {
      this.uiManager.removeAllModals();
      const intent = localStorage.getItem('downloadIntent');
      if (intent === 'ebook') {
        localStorage.removeItem('downloadIntent');
        this.openEbookViewer();
      }
  ...
  handleAccessEbook() {
    console.log('[App] Access E‑Book clicked');
    if (this.authManager.isAuthenticated()) {
      this.openEbookViewer();
    } else {
      // Remember intent and open Google sign‑in
      localStorage.setItem('downloadIntent', 'ebook');
      this.uiManager.showGoogleSignInModal(() => this.authManager.redirectToGoogleSignIn());
    }
  }
  ...
  handleSolutionsClick() {
    console.log('[App] Solutions download requested');
    this.uiManager.showSuccessNotification('Download Started!', 'Your solution file is downloading now');
    setTimeout(() => this.downloadManager.startDownload(), 500);
  }
}
