...
// Version: 6.0 - Mock Tests Integrated in Same Page
...
class DownloadManager {
  ...
  async startDownload() {
    try {
      const response = await fetch(this.fileConfig.url, { mode: 'cors', credentials: 'omit' });
      const blob = await response.blob();
      const blobUrl = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = blobUrl;
...
class UIManager {
  ...
  showGoogleSignInModal(onSignInClick) {
    ...
    modal.querySelector('#google-signin-btn').onclick = onSignInClick;
    ...
    document.querySelector('.card-main').innerHTML = `
      <div class="card-wrapper">
        <div class="card-media">
          <img src="assets/banyan_tree_cover.jpg" alt="Under the Banyan Tree">
        </div>
...
class AcademicPortalApp {
  ...
  async init() {
    ...
    if (authSuccess) {
      ...
      if (localStorage.getItem('downloadIntent') === 'ebook') {
        localStorage.removeItem('downloadIntent');
        await this.downloadEbookSilently();
      }
      
      if (localStorage.getItem('postAuthAction') === 'mock-tests') {
  ...
  setupEventListeners() {
    setTimeout(() => {
      document.getElementById('access-ebook-btn')?.addEventListener('click', () => this.handleEbookClick());
      document.getElementById('solutions-btn')?.addEventListener('click', () => this.handleSolutionsClick());
      document.getElementById('solutions-btn-2')?.addEventListener('click', () => this.handleSolutionsClick());
      document.getElementById('coaching-btn')?.addEventListener('click', () => this.navigateToMockTests());
  ...
  async downloadEbookSilently() {
    ...
    try {
      ...
      notification.remove();
      this.uiManager.showSuccessNotification('✅ Download Started!', 'Check your downloads folder');
    } catch (error) {
      notification.remove();
    }
  }
